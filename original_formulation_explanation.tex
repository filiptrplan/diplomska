...
\begin{itemize}
	\item Je referenca deljena (\textit{shared}), torej je oblike \verb|&x| in
	      \begin{itemize}
		      \item ustvarimo novo spremenljivo (\textit{mutable}) referenco (* tukaj ne vem točno kako bi formaliziral, moram še pogledati malo borrow checker rules *) \textbf{ali}
...
\begin{itemize}
	...
	\item $\mathcal{P}$: Množica vseh točk (points) v grafu poteka kontrol (control flow graph - CFG) programa. Točke so lahko na začetku ali sredi stavka.
	      \begin{itemize}
...
Te relacije predstavljajo začetni vhod za analizo. Izpeljane so iz sistema tipov in prejšnje analize v MIR.
\begin{itemize}
