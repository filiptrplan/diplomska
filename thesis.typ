...
// TODO:: posledicno je cudno

Eden najbolj razširjenih jezikov je C, kjer je programerju povsem prepuščeno upravljanje s pomnilnikom. Tovrsten pristop, imenovan ročno upravljanje s pomnilnikom, lahko vodi do izredno hitrih programov in krajših časov prevajanja v primerjavi z Rustom @glazarCodingRustBad2023. Vendar je obenem pogost vir napak @MemorySafetya.

Alternativni pristop ročnemu upravljanju je avtomatsko upravljanje s pomnilnikom, kjer programski jezik zagotavlja varno dodeljevanje in sproščanje pomnilnika. S tem razbremeni programerja, da se lahko osredotoči na pisanje programa, vendar imajo jeziki z avtomatskim upravljanjem s pomnilnikom zaradi zakasnjenega sproščanja večjo porabo pomnilnika. Da lahko čistilec pomnilnika najde pomnilniške lokacije za sprostitev, pride do premorov med izvajanjem programa ali zakasnitev ob vsaki operaciji @bakerListProcessingReal1978.

Rust pristopi k upravljanju s pomnilnikom na drugačen način. Veljavnost dostopanja do pomnilniških lokacij se preverja med časom prevajanja s pomočjo preverjevalnika izposoj #angl[borrow checker]. Ta je del Rustovega prevajalnika, ki se ukvarja s tokom podatkov in pomnilniškimi lokacijami. Rust imenuje zbirko pravil, ki opisuje delovanje preverjevalnika izposoj, lastništvo #angl[ownership]. V knjigi _The Rust Programming Language_ lastništvo opišejo tako: _"Ownership is a set of rules that govern how a Rust program manages memory"_ @klabnikRustProgrammingLanguage2023. Tak pristop ima dve glavni prednosti: zagotovi nam, da je naš program pomnilniško varen, kot pri avtomatičnem upravljanju s pomnilnikom, ter omogoči hitrost izvajanja programov, ki jo lahko dosežemo z ročnim upravljanjem pomnilnika @klabnikRustProgrammingLanguage2023. Pogosto omenjena slabost Rusta je dolg čas prevajanja @glazarCodingRustBad2023, ki sicer ni odvisen samo od preverjevalnika izposoj, vendar zagotovo njegov prispevek ni zanemarljiv.

V nadaljevanju bomo uporabljali dva podobna pojma. _Varen program_ je program, ki ne povzroča pomnilniških napak. _Veljaven program_ pa je program, ki ga Rustova pravila lastništva in izposojanja smatrajo kot varnega. Cilj Rustovega prevajalnika je, da sta ti dve množici programov enaki, vendar zaradi neizračunljivosti se izkaže, da je vsak veljavni program tudi varen, vendar ni vsak varen program veljaven.
...
NLL je bil natančno opisan v RFC-ju #angl[request for comment], kar je potem vodilo njegov razvoj. Polonius pa je nastal kot predlog na spletnem blogu enega izmed razvijalcev Rusta, kjer se je postopoma razvijal skozi nadaljne objave @PoloniusRevisitedPartb @PoloniusRevisitedPartc @WhatPoloniusPolonius. Celovit centraliziran formalen opis Poloniusa trenutno ne obstaja, le nekaj spletnih objav, delni formalni opis v magistrskem delu enega izmed razvijalcev @stjernaModellingRustsReference2020, nedokončana knjiga na GitHubu @WhatPoloniusPolonius ter trenutna implementacija v Rustovem prevajalniku.

Cilj te naloge je torej na svoj način formalizirati Poloniusova inferenčna pravila in nadgraditi obstoječo formalizacijo @stjernaModellingRustsReference2020. Najprej raziščemo pretekle poskuse formalizacije Rusta in sorodne načine upravljanja s pomnilnikom. Sledi intuitivni opis Rustovih pravili izposojanja in nato formalni opis Poloniusovih inferenčnih pravil.

== Motivacijski primer <chap:motivacijski-primer>

Za grajenje intuicije o razlikah med trenutno različico preverjevalnika izposoj (NLL) in med Poloniusom bomo obravnavali motivacijski primer (@listing:mot_ex), ki ga NLL zavrne in Polonius sprejme. Primer je prilagojen iz prvotnega predloga NLL-ja @2094nllRustRFC.
...
Če postopoma sledimo sporočilu o napaki na @listing:mot_ex_err[izpisku], lahko vidimo, kje NLL ne sprejme varnega programa. V vrstici 8 kličemo funkcijo `get_mut`, ki vrne unijo z dvema možnostima. Lahko vrne spremenljivo referenco na vrednost, ki pripada ključu (`Some(value)`), ali pa ne vrne ničesar (`None`). Če vrne vrednost, je spremenljivka `map` začasno izsposojena (torej obstaja spremenljiva referenca na pomnilniško lokacijo z njenimi podatki), kar se zgodi v vrstici 9. Vendar NLL smatra, da je spremenljivka `map` še vedno izsposojena, tudi če ne vrnemo njene reference iz funkcije `get_mut` (v vrsticah 11-13). Ko torej poskušamo vstaviti nov par v `map`, nam to preverjevalnik izposoj NLL konzervativno prepreči, saj operacija `insert` zahteva spremenljivo referenco na spremenljivko `map` (saj jo spreminjamo z vstavljanjem para), dve spremenljivi referenci na isto mesto pa po pravilih jezika ne smeta obstajati.
...
Rust je jezik inžinirjev, ne raziskovalcev. Od začetka je bil zasnovan tako, da reši današnje praktične probleme in se ukvarja s specifikacijami in formalnostjo kasneje. Ta način dela je porodil veliko vprašanj o temu, kako jezik deluje, zakaj deluje in ali deluje pravilno. Čeprav je Rust prišel na svet šele leta 2015 @4YearsRust, je v zadnjem desetletju nastalo vrsto člankov o raznih formalnih pogledih na Rust.
...
+ *Polonius v akademskem svetu in praksi:* tukaj bomo opisali, kako je Polonius nastal, kje trenutno stoji in drugo literaturo o njem.

== Poskusi formalizacije Rusta

Formalizacija Rusta in preverjanje pravilnosti kode je zelo relevantna tema, saj je jezik kompleksen in brez uradne specifikacije. Osredotočili se bomo na formalizacije preverjevalnika izposoj, saj je to tema te naloge. Obstajajo tudi številne druge formalizacije, ki se pa ukvarjajo s sistemom tipov ali s pravilnostjo programov @matsushitaRustHornBeltSemanticFoundation2022a @villaniTreeBorrows2025a.
...
Eden izmed ključnih del na področju formalizacije je akademski članek RustBelt Junga idr., ki so zasnovali jezik imenovan lambdaR ter ga opremili s semantičnim modelom imenovan RustBelt. Jezik lambdaR je sam bolj podoben MIRu kot pa izvirni kodi Rusta. Vsebuje tudi sistem tipov in pravila sklepanja, ki modelirajo Rustov MIR. Končajo z dokazom, da katerikoli lambdaR program, ki je semantično in tipsko pravilen, ne bo končal v zataknjenem stanju @jungRustBeltSecuringFoundations2018.

// TODO: zataknejo

Še en model Rusta je imenovan Weissov idr. Oxide @weissOxideEssenceRust2019, kjer zasnujejo višjenivojski jezik, tokrat bolj podoben izvirni kodi Rusta. V primerjavi z RustBelt, se avtorji bolj osredotočijo na preverjevalnik izposoj, ker niso želeli natančno modelirati operacijske semantike, vendar so želeli povzeti bistvo Rusta (po njihovih besedah). Oxidova sintaksa je zelo podobna Rustovi, samo da so vsi tipi eksplicitno podani. Avtorji nadaljujejo članek s tem, da podajo pravila sklepanja nad tem sistemom tipov in uvedejo pojem _approximate provenances_, ki je njihov način izražanja regij, kot so zastavljene v NLL-ju. Članek se nadaljuje s semantiko majhnih korakov in konča s formalnim dokazom, da se pravilno konstruirani programi v Oxidu ne zataknejo. Pri tem članku je še zanimivo, da specifično omenijo Polonius in povejo, da je Poloniusov model regij zelo podoben njihovim _approximate provenances_. Omenijo, da kljub pomanjkanju raziskave povezave med Polonius in Oxidom, lahko na Oxide gledamo kot formulacijo Poloniusa preko sistema tipov.

// TODO: framework?
// TODO: zataknjenem

Takih podobnih modelov je še mnogo, tukaj jih bomo omenili še par za celostnost. Članek Crihchtona idr. zastavi poenostavljen pedagoški model za razumevanja sistema lastništva @crichtonGroundedConceptualModel2023. Zanimivo je, da v njemu tudi eksplicitno opišejo Polonius, vendar na kratko. Patina @reedPatinaFormalizationRust je eden izmed prvih formalnih semantičnih modelov za Rust in je nastala še pred uvedbo NLL-ja. KRust @wangKRustFormalExecutable2018 je formalni izvedljiv semantični model v frameworku(?) K in tudi, v kolikor vemo, prvi semantični model v dobi NLL-a. Featherweight Rust @pearceLightweightFormalismReference2021 se eksplicitno ukvarja s formalizacijo preverjevalnika izposoj (vrste NLL), tako da definira slovnico in semantiko majhnih korakov. Zasnovan je bil kot striktna podmnožica Rustove sintakse. Še zadnji članek, ki ga bomo omenili je od Ho idr., ki so dokazali, da LLBC (low-level borrow calculus, model rustovega MIR), res pravilno modelira Rust in ne konča v zataknjenem stanju.

== Modeli sorodni lastništvu

V temu poglavju se bomo osredotočili na t.i. _regijsko upravljanje s pomnilnikom_ #angl[region-based memory management] @tofteRegionBasedMemoryManagement1997, ki sta ga prvo opisala Tofte in Talpin. Ta model upravljanja s pomnilnikom lahko razumemo skoraj kot direktni predhodnik lastništva.

Njuna poglavitna motivacija je bila, da najdeta kompromis med striktno eksplicitnim upravljanjem s pomnilnikom, kot pri C, in avtomatskim čiščenjem pomnilnika, kot pri Javi. Za navdih sta vzela delovanje sklada, kjer se spomin dodeli na začetku okvirja ter sprosti na koncu. Iz tega sta ustvarila koncept regij, ki so dodatne označbe poleg tipov in podajo informacije o tem kdaj se vrednost mora sprostiti.

Ker bi bilo anotiranje vsake vrednosti z regijami nepraktično, sta uvedla sistem inferenciranja teh regij, podoben tistemu, ki inferecira življenjske dobe v Rustu. V delu definirata visokonivojski jezik `SExp` podoben SML-u skupaj s sistemom ML tipov in semantiko majhnega koraka. Nato uvedeta jezik `TExp` v katerega se `SExp` pretvarja. Ključna razlika med njima je, da ima `TExp` regijske anotacije, `SExp` pa ne. Delo nadaljujeta s sistemom za avtomatično inferenciranje teh regij osnovnanem na Milnerjevemu sistemu tipov. Končata z dokazi o pravilnosti njunega sistema in pravilnosti prevoda med `SExp` in `TExp`.

// TODO: memory leaks

Rust sicer ni bil prvi jezik, ki je uvedel soroden pristop upravljanja s pomnilnikom (poleg seveda akademskega jezika predstavljenega v izvornem delu). Eden izmed najbolj znanih jezikov, ki so v praksi uporabili regijsko upravljanje s pomnilnikom, je Cyclone @grossmanRegionBasedMemoryManagement. Ustvarjen je bil kot dopolnilo C-ju z raznimi bolj naprednimi tipi. Kasneje so tudi dodali regijsko upravljanje s pomnilnikom, ki ga lahko programer doda C programu z nekaj dodatnimi regijskimi anotacijami. Prva implementacija sicer ni bila popolna in je še vedno kdaj uvedla memory leaks(??), vendar so kasnejše različice jezika z linearnimi regijami to poskušale popraviti @fluetLinearRegionsAre2006.

== Polonius v akademskem svetu in praksi

Polonius je bil prvotno formuliran v spletni objavi N. D. Matsakisa, kjer je poljudno pojasnil, kako bi Polonius naslovil problem starega NLL, in podal osnovno formulacijo v Datalogu @matsakisAliasbasedFormulationBorrow. Delo se je nato nadaljevalo v GitHub repositoriju `rust-lang/polonius` @RustlangPolonius2025, kjer so to originalno formulacijo implementirali v Rust kodi.

// TODO: prej omenjenem oxidu?

Leta #cite(<stjernaModellingRustsReference2020>, form: "year") je Amanda Stjerna v svojem magistrskem delu podala prvo matematično formulacijo Poloniusa kot sistema tipov @stjernaModellingRustsReference2020. Ta formulacija je bila močno osnovana na prej omenjenemu Oxidu, saj sta si modela zelo podobna. V svojem delu je tudi opisala pravila za preverjevalnik posoj, ki jih kasneje v nalogi tudi opišemo in formaliziramo. Njeno delo se nadaljuje z bolj natančnim opisom Poloniusovega notranjega delovanja z vsemi podrobnostmi potrebnimi za konkretno implementacijo. V kolikor vemo, je to delo eno izmed najbolj podrobnih in celovitih opisov Poloniusovega delovanja.

// TODO: situacija pocasi spreminja?

Trenutno še ne obstaja uradna specifikacija za Polonius, vendar se situacija počasi spreminja. Leta 2025 je bil Polonius implementiran v glavni veji Rustovega prevajalnika in je trenutno dostopen v `nightly` različici prevajalnika za zastavico @ScalablePoloniusSupporta. Če pogledamo izvorno kodo prevajalnika, lahko vidimo da obstajata dve različici Poloniusa @RustlangRust2026: `legacy` različica podobna prvotni implementaciji iz @RustlangPolonius2025 ter t.i. `alpha` različica, ki je nastala zaradi omejitev glede hitrosti izvajanja `legacy` različice. `alpha` različica je napisana v Rustu in ne poskuša emulirati Dataloga, vendar ji manjkajo nekatere sposobnosti polne različice @ScalablePoloniusSupporta.

#show "amir": `a-mir-formality`

Pomanjkanje uradne specifikacije je problem, ki ga trenutno rešuje Rustova ekipa za tipe v okviru projekta imenovanem amir @BorrowCheckingAmirformalityb @RustlangAmirformality2026. Ekipa želi ustvariti uradno izvedljivo specifikacijo za Rust, s katero se bo potem preverjalo pravilno delovanje Rustovega prevajalnika. "Izvedljiva" v tem kontekstu pomeni, da ji lahko kot vhod damo Rust program (oz. MiniRust, ki je bolj podoben MIR @MinirustMinirust2026), amir pa ga sprejme oz. zavrne, glede na to ali je pravilno tipiziran in pomnilniško varen. V drugi polovici leta 2025 se je začelo delo na specifikaciji za `alpha` različico Poloniusa, ki je v času pisanja (začetek 2026) že skoraj končana. Če se delo pod amir nadaljuje, bo Rust lahko končno dobil uradno specifikacijo, ki mu že od spočetja manjka.
